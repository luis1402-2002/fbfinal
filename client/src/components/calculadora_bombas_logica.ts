// ========================================================================
// Arquivo de Lógica da Calculadora de Bombas FB (TypeScript)
// ========================================================================
...
export interface CalculationResult {
  ...
  pumpType: string;                 // Tipo da bomba recomendada (FBCN, FBOT, FBE)
  efficiency: number;              // Eficiência no ponto de operação (%)
  power: number;                   // Potência no ponto de operação (kW)
  npsh: number;                    // NPSH no ponto de operação (m)
  flowRate: number;                // Vazão no ponto de operação (m³/h)
  head: number;                    // Altura no ponto de operação (m)
  points: CombinedPoint[];         // Pontos combinados para o gráfico
  curvePoints: PumpCurvePoint[];   // Pontos da curva da bomba
  rotationSpeed: number;           // Rotação utilizada no cálculo (RPM)
  specificRecommendations?: string[]; // Recomendações específicas
}
...
export function calculateIdealPump(input: CalculationInput): CalculationResult | null {
  try {
    ...
    // 7. Montar objeto de resultado
    const result: CalculationResult = {
      pumpModel: recommendedModel,
      pumpType: input.selectedPumpType,
      efficiency: operationPoint.efficiency,
      power: operationPoint.power,
      npsh: operationPoint.npsh,
      flowRate: operationPoint.flow,
      head: operationPoint.pumpHead,
      points: combinedPoints,
      curvePoints: pumpCurvePoints,
